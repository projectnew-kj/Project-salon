...
import { Calendar, DollarSign, Clock, CheckCircle2, ChevronRight, Scissors } from 'lucide-react-native';
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={[styles.adminName, { color: colors.text }]}>{admin?.name || 'Administrator'}</Text>
      </View>
      ...
      <View style={styles.metricsGrid}>
        <View style={[styles.metricCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <Text style={[styles.metricValue, { color: colors.text }]}>{metrics.todayBookings}</Text>
          <Text style={[styles.metricLabel, { color: colors.textSecondary }]}>Today's Queue</Text>
        </View>
        ...
        <View style={[styles.metricCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <Text style={[styles.metricValue, { color: colors.text }]}>{metrics.pendingBookings}</Text>
          <Text style={[styles.metricLabel, { color: colors.textSecondary }]}>Pending Actions</Text>
        </View>
        ...
        <View style={[styles.metricCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <Text style={[styles.metricValue, { color: colors.text }]}>₹{metrics.totalRevenue.toLocaleString()}</Text>
        ...
        <View style={[styles.metricCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <Text style={[styles.metricValue, { color: colors.text }]}>{metrics.completedBookings}</Text>
      ...
      <View style={styles.quickActionContainer}>
        ...
        >
          ...
          <Text style={[styles.quickActionText, { color: colors.text }]}>Manage Services</Text>
        </TouchableOpacity>
        ...
        >
          ...
          <Text style={[styles.quickActionText, { color: colors.text }]}>Hours & Slots</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.sectionHeader}>
        ...
        <TouchableOpacity onPress={() => router.push('/(tabs)/bookings')}>
          <Text style={[styles.seeAllText, { color: colors.primaryAccent }]}>View All</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.listContainer}>
        {bookings.slice(0, 5).map((item) => (
          <TouchableOpacity
            key={item._id}
            activeOpacity={0.7}
            style={[styles.bookingRow, { backgroundColor: colors.surface, borderColor: colors.border }]}
          ...
          >
            <View style={styles.bookingLeft}>
              ...
              <Text style={[styles.customerName, { color: colors.textSecondary }]}>{item.user?.name || 'Customer'}</Text>
              <Text style={[styles.serviceTitle, { color: colors.textSecondary }]}>
                {item.haircut?.name || item.offer?.title || 'Service'} • {item.bookingTime || 'Anytime'}
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 42, height: 42, borderRadius: 10, alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
